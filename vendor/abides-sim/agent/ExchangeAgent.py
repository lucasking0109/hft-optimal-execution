  ...
  def publishOrderBookData(self):
    ...
    for agent_id, params in self.subscription_dict.items():
      for symbol, values in params.items():
        ...
        if (freq == 0) or \
           ((orderbook_last_update > last_agent_update) and ((orderbook_last_update - last_agent_update).value >= freq)):  # Phase 5B+ patch: pandas Timedelta.delta deprecated → .value (nanoseconds)
